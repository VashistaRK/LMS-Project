...
export const submitTest = async (req, res) => {
  try {
      ...
      if (section.type === "mcq") {
        for (let q of section.questions) {
          ...
          if (
            ...
            String(submittedAnswer) === String(dbQ.answer)
          ) {
            totalScore += 1;
          }
        }
    ...
    let startedCourse = user.startedCourses.find(
    ...
    if (!startedCourse) {
      ...
      user.startedCourses.push(startedCourse);
    }

    if (typeof startedCourse.scores.set === "function") {
      startedCourse.scores.set(testId, totalScore);
    } else {
      startedCourse.scores[testId] = totalScore;
    }
