...
router.get("/", requireAdmin, async (req, res) => {
  try {
    const [users, courses] = await Promise.all([
      User.find({}), // ✅ No populate since startedCourses are strings
      Course.find({}),
    ]);
    ...
    const courseEnrollments = courses.map((c) => {
      const count = users.filter(
        (u) => (u.startedCourses || []).includes(c.id), // ✅ Match on Course.id
      ).length;
    ...
    const courseActivity = users
      .map((u) => ({
        user: u.name || u.email,
        courseCount: (u.startedCourses || []).length,
      }))
