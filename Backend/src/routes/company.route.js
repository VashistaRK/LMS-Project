...
router.put("/admin/:slug", requireAdmin, async (req, res) => {
  try {
    ...
    const comp = await Company.findOneAndUpdate({ slug }, update, {
      new: true,
    });
    if (!comp) return res.status(404).json({ error: "Company not found" });
    res.json(comp);
  ...
  async (req, res) => {
    try {
      ...
      if (!title || !year || !file || !file.buffer) {
        return res
          .status(400)
          .json({ error: "title, year and file are required" });
      }
...
router.post("/admin/:slug/tests", requireAdmin, async (req, res) => {
  try {
    ...
    if (!testId || !title || !Array.isArray(sections)) {
      return res
        .status(400)
        .json({ error: "testId, title and sections are required" });
    }
    const comp = await Company.findOne({ slug });
    if (!comp) return res.status(404).json({ error: "Company not found" });
    ...
    for (const s of sections) {
      ...
      if (Array.isArray(s.questionIds) && s.questionIds.length > 0) {
        let count = 0;
        if (s.key === "mcq") {
          count = await QuizQuestion.countDocuments({
            _id: { $in: s.questionIds },
          });
        } else if (s.key === "coding") {
          // validate against your coding question collection
          count = await CodingQuestion.countDocuments({
            _id: { $in: s.questionIds },
          });
        } else {
          // essay / descriptive: either no validation or validate in a different way
          count = 0;
        }

        if (count !== s.questionIds.length) {
          // helpful debug: return which IDs are missing (optional)
          return res
            .status(400)
            .json({ error: "One or more questionIds invalid in section" });
        }
      }
    }
    ...
    res
      .status(201)
      .json({ ok: true, test: comp.tests.find((t) => t.testId === testId) });
  } catch (err) {
...
router.post("/:slug/tests/:testId/start", async (req, res) => {
  try {
    ...
    const mcqDocs = await QuizQuestion.find({ _id: { $in: allIds } }).lean();
    const codeDocs = await CodingQuestion.find({ _id: { $in: allIds } }).lean();

    // merge and map
    const qDocs = [...mcqDocs, ...codeDocs];
...
router.post("/:slug/tests/:testId/submit", async (req, res) => {
  try {
    ...
    const mcqDocs = await QuizQuestion.find({
      _id: { $in: submittedBankIds },
    }).lean();
    const codeDocs = await CodingQuestion.find({
      _id: { $in: submittedBankIds },
    }).lean();
    const qDocs = [...mcqDocs, ...codeDocs];
    ...
    for (const s of test.sections) {
      const secResponses = Array.isArray(responses[s.key])
        ? responses[s.key]
        : [];
      const secDetail = [];
      for (const r of secResponses) {
        ...
        if (q.type === "mcq" || q.type === "MCQ") {
          ...
          const submitted =
            typeof r.value === "number" ? Number(r.value) : Number(r.value);
          const correct =
            !Number.isNaN(submitted) && submitted === correctIndex;
