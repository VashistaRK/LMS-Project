...
export default function TestPage() {
  ...
  // Simple toast system (bottom-right) used instead of `alert`/`confirm`
  type Toast = {
    id: number;
    message: string;
    variant?: "info" | "error" | "success" | "warning";
    persistent?: boolean; // if true, won't auto-dismiss and may contain actions
    resolve?: (value: boolean) => void; // for confirm to resolve user choice
  };
  ...
  const doStart = async () => {
    ...
    try {
      ...
      showToast("Failed to start test. Returning to practice list.", "error");
      try {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        showToast("Failed to load test. Returning to practice list.", "error");
        try {
          navigate("/freshers-pratice");
        } catch {}
      }
  ...
  const handleSubmit = async () => {
    if (!attemptId) {
      showToast("No active attempt to submit.", "warning");
      try {
        navigate("/freshers-pratice");
      } catch {}
      return;
    ...
    try {
      ...
      try {
        showToast(
          `Assessment submitted: ${res.score ?? 0} / ${res.total ?? 0}`,
          "success"
        );
      } catch {}
      ...
      showToast("Failed to submit attempt. Please try again.", "error");
    }
  };
                  ...
                  <div className="sticky top-6">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {questions.map((_q, idx) => {
                        ...
                        const detail =
                          resultDetails && Array.isArray(resultDetails)
                            ? resultDetails[idx]
                            : null;
                        ...
                        if (isCurrent)
                          cls += " ring-2 ring-offset-1 ring-blue-300";
                          ...
                          <div>
                            ...
                            {showAnswers && (
                              <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                                ⚠ This question requires manual grading and is
                                not included in the auto-calculated score.
                              </div>
                            )}
                          </div>
                        )}
        ...
        <div className="fixed right-4 bottom-4 z-50 flex flex-col items-end gap-3">
          {toasts.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className={`max-w-sm w-full p-3 rounded shadow ${variantCls}`}
              >
                <div className="flex items-start gap-3">
                  <div className="flex-1 text-sm">{t.message}</div>
                  <div className="flex-shrink-0 flex gap-2">
                    {t.persistent && t.resolve ? (
                      <>
                        <button
                          onClick={() => {
                            try {
                              t.resolve && t.resolve(true);
                            } catch {}
                            setToasts((s) => s.filter((x) => x.id !== t.id));
                          }}
                          className="px-3 py-1 rounded bg-white text-gray-800 font-semibold"
                        >
                          Confirm
                        </button>
                        <button
