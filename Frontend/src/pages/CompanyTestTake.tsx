...
export default function CompanyTestTake() {
  ...
  const [responses, setResponses] = useState<
    Record<string, Array<{ bankId: string; value: any }>>
  >({});
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        console.log("Started company test:", started);
        ...
        if (secs.length > 0) {
          ...
          setSelectedQuestionIndex(
            secs[0].questions && secs[0].questions.length ? 0 : -1
          );
        }
      } catch (err) {
  ...
  const currentSectionObj = useMemo(
    () => sections.find((s) => s.key === selectedSection) ?? null,
  ...
  const setAnswer = (sectionKey: string, bankId: string, value: any) => {
    setResponses((prev) => {
      ...
      const foundIdx = arr.findIndex(
        (it) => String(it.bankId) === String(bankId)
      );
      if (foundIdx === -1) {
  ...
  const getSubmittedValue = (sectionKey: string, bankId: string) => {
    const arr = Array.isArray(responses[sectionKey])
  ...
  const calculateScoreFromResponses = useMemo(() => {
    ...
    const sectionScores: Record<
      string,
      { earned: number; total: number; details: any[] }
    > = {};
    ...
    sections.forEach((section) => {
      ...
      section.questions.forEach((q: any) => {
        ...
        if (q.type === "MCQ" || q.type === "mcq") {
          ...
          const sectionResponses = Array.isArray(responses[section.key])
            ? responses[section.key]
            : [];
          const found = sectionResponses.find(
            (r) => String(r.bankId) === String(q.bankId)
          );
  ...
  const handleSubmit = async () => {
    ...
    const ok = window.confirm(
      "Submit test? You will not be able to change answers afterwards."
    );
    if (!ok) return;
    setLoading(true);
    try {
      ...
      const frontendResult = {
        ...
        details: scoreResult
          ? Object.keys(scoreResult.sectionScores).reduce((acc, key) => {
      ...
      try {
        ...
        console.warn(
          "Failed to submit to server, but score calculated locally",
          err
        );
      }
  ...
  const currentQuestion =
    currentSectionObj?.questions?.[selectedQuestionIndex] ?? null;
            ...
            <div className="bg-gradient-to-r from-[#C21817] to-[#A51515] rounded-2xl shadow-lg p-6 text-white mb-6">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  {calculateScore &&
                  (calculateScore.total > 0 || calculateScore.earned > 0) ? (
                    <>
                    ...
                    <>
                      ...
                      <div className="text-lg opacity-90">
                        {(result.total ?? 0) > 0
                          ? `${Math.round(
                              ((result.score ?? 0) / (result.total ?? 1)) * 100
                            )}%`
                          : "0%"}
                      </div>
                ...
                <div className="text-right">
                  ...
                  {calculateScore &&
                  Object.keys(calculateScore.sectionScores).length > 0 ? (
                    sections
                      .map((s) => {
                        ...
                        return (
                          <div key={s.key} className="text-sm mb-1">
                            {s.title}: {secScore.earned} / {secScore.total}{" "}
                            points
                          </div>
                        );
                      })
              ...
              <div className="mt-4 pt-4 border-t border-white/20 text-sm opacity-90">
                Note: Score is calculated based on section points. Only MCQ
                questions are auto-graded. Descriptive and coding questions
                require manual grading.
                {(!calculateScore || calculateScore.total === 0) && (
                  <div className="mt-2 text-xs">
                    Debug: Open browser console (F12) to see detailed
                    calculation logs.
                  </div>
                )}
              </div>
              ...
              <div className="space-y-3">
                {sections.map((s) => {
                  ...
                  const answeredCount = sectionQuestions.filter(
                    (q: any) => getSubmittedValue(s.key, q.bankId) !== undefined
                  ).length;
                        ...
                        <div className="p-3 bg-gray-50 rounded-b-lg border-t border-gray-200">
                          ...
                          <div className="grid grid-cols-5 gap-2">
                            {sectionQuestions.map((q: any, qi: number) => {
                              const submitted = getSubmittedValue(
                                s.key,
                                q.bankId
                              );
                              const isCurrent = qi === selectedQuestionIndex;
                              const det = result?.details?.[s.key]?.find(
                                (d: any) =>
                                  String(d.bankId) === String(q.bankId)
                              );
                              ...
                              if (result) {
                                if (isCorrect)
                                  ...
                                  det &&
                                  typeof det.correctIndex !== "undefined"
                                )
                                  bgColor =
                ...
                <>
                  ...
                  <div className="mb-6 pb-4 border-b border-gray-200">
                    ...
                    <div className="flex items-center gap-4 text-sm text-gray-600">
                      <span>
                        Points: <strong>{currentQuestion.points ?? 1}</strong>
                      </span>
                      <span>
                        Type: <strong>{currentQuestion.type}</strong>
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="mb-6">
                    ...
                    {(() => {
                      const det = result?.details?.[
                        ...
                        (d: any) =>
                          String(d.bankId) === String(currentQuestion.bankId)
                      );
                        ...
                        <div className="space-y-3">
                          {currentQuestion.options?.map(
                            (opt: string, oi: number) => {
                              ...
                              const userVal = getSubmittedValue(
                                currentSectionObj.key,
                                currentQuestion.bankId
                              );
                              const selected =
                                userVal !== undefined && Number(userVal) === oi;
                              const isCorrectOption =
                                det &&
                                typeof det.correctIndex !== "undefined" &&
                                Number(det.correctIndex) === oi;
                              const userSubmittedIndex =
                                det && typeof det.submitted !== "undefined"
                                  ? Number(det.submitted)
                                  : undefined;
                        ...
                        <div>
                          ...
                          {result && (
                            <div className="mt-3 p-3 bg-yellow-50 border border-yellow-200 rounded-lg text-sm text-yellow-800">
                              ⚠ This question requires manual grading and is not
                              included in the auto-calculated score.
                            </div>
                          )}
                        </div>
                      );
                            ...
                            <div className="mt-3 pt-3 border-t border-gray-300">
                              ...
                              {currentQuestion.points && (
                                <span className="ml-4 text-gray-600">
                                  Points:{" "}
                                  {det.correct ? currentQuestion.points : 0} /{" "}
                                  {currentQuestion.points}
                                </span>
                              )}
                            </div>
                      ...
                      onClick={() => {
                        if (selectedQuestionIndex > 0) {
                          ...
                          if (currentIdx > 0) {
                            ...
                            setSelectedQuestionIndex(
                              (prevSection.questions?.length ?? 1) - 1
                            );
                          }
                        }
                      }}
                      disabled={
                        selectedSection === sections[0]?.key &&
