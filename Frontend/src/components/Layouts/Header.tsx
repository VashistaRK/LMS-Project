            ...
            {suggestions.length > 0 && (
              <div className="absolute top-full left-0 w-full bg-white/90 backdrop-blur-md border border-white/40 rounded-xl mt-1 shadow-xl z-50">
                {suggestions.map((item, i) => (
                  <button
                    key={i}
                    className="w-full text-left px-4 py-2 hover:bg-white/40 flex justify-between"
        ...
        {!user ? (
          ...
          <div className="relative" ref={userMenuRef}>
            <button
              ...
              className="flex items-center space-x-3 px-3 py-2 rounded-xl hover:bg-white/30 transition"
            >
              <img
                src={user?.picture}
                alt={user?.name ?? "user"}
            ...
            <AnimatePresence>
              {isUserMenuOpen && (
                ...
                >
                  ...
                  <a
                    href="/profile"
                    className="flex items-center px-4 py-2 text-sm hover:bg-white/40"
                  >
                  ...
                  <button
                    ...
                    className="w-full text-left px-4 py-2 text-sm text-red-600 hover:bg-red-50/60"
                  >
                    Sign Out
                  </button>
                </motion.div>
          ...
          >
            <div className="px-4 py-3 z-10 space-y-2">
              ...
              {!user ? (
                <a
                  href="/Authenticate"
                  className="block px-3 py-2 bg-gradient-to-r from-amber-400 to-yellow-500 text-white rounded-lg text-center"
                >
                  Sign In
                </a>
              ) : (
                <button
                  onClick={handleLogout}
                  className="w-full text-left px-3 py-2 rounded-lg text-red-600 hover:bg-red-50/60"
                >
                  Sign Out
                </button>
              )}
      ...
      <AnimatePresence>
        {showLogoutPopup && (
          <motion.div
            ...
            exit={{ opacity: 0, y: 30 }}
            transition={{ duration: 0.25 }}
            className="fixed bottom-6 right-6 bg-green-500 text-white px-4 py-2 rounded-xl shadow-lg"
          >
